...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().privateKeysDisabled()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed optical Bitcoin Transaction (PSoBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
